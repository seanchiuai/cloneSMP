...
- **Browser bridge** (`server/routes/api/voice/browser-stream.ts`): WebSocket for browser voice mode (PCM16 24kHz) with optional Hedra live avatar — routes audio to avatar via DataStream, manages LiveKit video subscription
- **Avatar** (`server/utils/avatar/hedra.ts`): Hedra lip-sync avatar session manager — room creation, API call, DataStream audio routing, readiness gating, preprocessed image caching with SSRF protection, warm session pool (release/acquire for reuse across calls)
- **Audio resampling** (`server/utils/avatar/audioResample.ts`): PCM16 downsampling 24kHz->16kHz with anti-aliasing FIR filter
...
- `server/utils/avatar/` — Hedra avatar integration (session manager, audio resampling)
