...
Browser --- WebSocket --->|  browser-stream.ts ---> orchestrator.ts            |
  (PCM16 24kHz base64)    |                          +-- deepgram-stt.ts       |
                          ...
                          |                          |                         |
                          |                          +---> hedra.ts (avatar)   |
                          |                             DataStream audio route |
                          |                             (PCM16 24kHz->16kHz)   |
                          |                                    |               |
Twilio ---- WebSocket --->|  stream.ts ---> orchestrator.ts    v               |
  (mulaw 8kHz base64)     |   (audio-convert: mulaw <-> PCM24k)               |
                          +---------------------------------------------------+
                                                               |
                          Client <--- LiveKit --- Hedra Avatar (video+audio track)
```

### Live Avatar (Hedra)
Server-side lip-sync avatar via Hedra API + LiveKit. When `withVideo: true` is sent in `session.start` and the spark has a `profileImageUrl`, the server:
1. Checks the **session pool** for a warm (reusable) Hedra session for this spark
2. If no warm session: creates a LiveKit room, joins as participant, calls Hedra API
3. Routes TTS audio (downsampled 24kHz->16kHz via FIR filter) to Hedra via LiveKit DataStream (`lk.audio_stream` topic)
4. Sends `avatar.ready` with LiveKit token/URL to the browser
5. Browser subscribes to Hedra's video+audio tracks via `useHedraAvatar.ts` (requests HIGH quality + 512×512 dimensions)
6. On normal call end, session is **released** to the pool (kept warm for 5 min); on error, session is **destroyed**
All avatar operations are non-fatal — voice always works without avatar.

**Session Pool**: Up to 10 warm sessions are held in-memory per spark ID. A warm session keeps the LiveKit room + Hedra avatar participant alive so the next call to the same spark skips room creation and Hedra API startup. Sessions auto-destroy after 5 min idle.
...
| `CLAUSE_BOUNDARY` | `/[.!?;:,—](?:\s\|$)/` | server constants.ts |
| `MIN_WORDS_PER_CHUNK` | 4 | server constants.ts |
...
| `SPECULATIVE_MIN_WORDS` | 5 | server constants.ts |
| `SPECULATIVE_TIMEOUT_MS` | 5000 | server constants.ts |
| `SPECULATIVE_SIMILARITY_THRESHOLD` | 0.8 | server constants.ts |
...
model Spark {
  ...
  profileImageUrl       String?   @map("profile_image_url")    // User-supplied spark avatar
  videoCallImageUrl     String?   @map("video_call_image_url") // Optimized 16:9 image for Hedra avatar
}
```
...
|----------|--------|-------------|
| `/api/voice/browser-stream` | WebSocket | Browser real-time voice with optional Hedra avatar (crossws) |
...
| `HEDRA_API_KEY` | Hedra avatar API | hedra.ts |
| `LIVEKIT_URL` | LiveKit Cloud WebSocket URL | hedra.ts |
| `LIVEKIT_API_KEY` | LiveKit API key | hedra.ts |
| `LIVEKIT_API_SECRET` | LiveKit API secret (JWT signing) | hedra.ts |
...
### Avatar
- `server/utils/avatar/hedra.ts` -- Hedra session manager: room creation, API call, DataStream audio routing, readiness gating, preprocessed image caching with SSRF protection, warm session pool with release/acquire lifecycle (~1024 lines)
- `server/utils/avatar/audioResample.ts` -- PCM16 24kHz->16kHz downsampling with 16-tap FIR low-pass filter (~114 lines)

### WebSocket Handlers
- `server/routes/api/voice/browser-stream.ts` -- Browser WS handler with voice profile, prosody, avatar session (release on normal close, destroy on error), LiveKit token (~352 lines)
...
- `composables/voice/useVoiceMode.ts` -- Voice session lifecycle, WebSocket management, auto-reconnect, VAD interrupt, avatar startup (472 lines)
- `composables/voice/useHedraAvatar.ts` -- LiveKit video subscriber: connects to room, attaches video/audio tracks, requests HIGH quality + 512×512 dimensions (~195 lines)
- `composables/voice/avatarRefs.ts` -- Shared reactive refs for avatar state (active, connecting, error)
...
- `components/workspace/voice/VideoAvatar.vue` -- Avatar UI component with connecting/active/error states (~204 lines)
...
- `tests/integration/voice-avatar-lifecycle.test.ts` -- Avatar lifecycle integration test
- `tests/unit/avatar/audioResample.test.ts` -- Audio resampling unit tests
- `tests/unit/avatar/hedra.test.ts` -- Hedra session manager unit tests
- `tests/unit/composables/useHedraAvatar.test.ts` -- LiveKit subscriber unit tests
