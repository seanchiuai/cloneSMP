...
| Event | Action |
|-------|--------|
| `checkout.session.completed` | Creates team record via `createOrUpdateTeamWithSubscription()`, sends Slack purchase notification via `notifyPurchase()` |
...
Internal team notifications via Slack webhooks (`server/utils/notifications.ts`).

### Functions
| Function | Trigger |
|----------|---------|
| `sendNotification(message, webhookUrl?)` | Generic message to Slack webhook |
...
### Configuration
- `SLACK_WEBHOOK_URL` -- runtime config `slackWebhookUrl`
...
| Variable | Purpose |
|----------|---------|
| `SLACK_WEBHOOK_URL` | Slack webhook for internal team notifications |
...
- `server/utils/notifications.ts` -- Slack webhook notifications
