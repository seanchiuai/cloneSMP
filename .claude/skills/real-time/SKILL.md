...
- Client messages: `session.start` (with JWT token, sparkId, locale, optional `withVideo`), `audio`, `interrupt`, `mute`, `ping`, `end`
- Server messages: `session.ready`, `audio`, `transcript.user`, `utterance.user` (complete user utterance), `transcript.ai`, `ai.speaking`, `ai.done`, `avatar.ready` (livekitToken, livekitUrl), `avatar.error` (message), `pong`, `error`
...
- Optional Hedra live avatar: when `withVideo: true`, starts avatar session via `server/utils/avatar/hedra.ts`, routes TTS audio via LiveKit DataStream, sends `avatar.ready` with LiveKit credentials for client video subscription
...
- `composables/voice/useHedraAvatar.ts` -- Client LiveKit video subscriber for Hedra avatar
- `server/utils/avatar/hedra.ts` -- Hedra avatar session manager (LiveKit room, DataStream audio)
